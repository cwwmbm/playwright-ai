...
export async function takeAction(
	...
	try {
		switch (action.action) {
			...
			case "right_click":
				await page.mouse.click(action.coordinate?.[0] ?? 0, action.coordinate?.[1] ?? 0, { button: "right" });
				break;

			case "middle_click":
				await page.mouse.click(action.coordinate?.[0] ?? 0, action.coordinate?.[1] ?? 0, { button: "middle" });
				break;

			case "double_click":
				await page.mouse.dblclick(action.coordinate?.[0] ?? 0, action.coordinate?.[1] ?? 0);
				break;
			...
			case "cursor_position":
				const position = await page.mouse.move(action.coordinate?.[0] ?? 0, action.coordinate?.[1] ?? 0);
